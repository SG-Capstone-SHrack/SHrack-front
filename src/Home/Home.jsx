...
const HomePage = () => {
  ...
  const handleLogout = () => { //로그아웃 시 : 로컬스토리지에서 auth 삭제 후 홈으로 이동
    localStorage.removeItem('auth');
    window.location.href = '/';
  };
  useEffect(() => { //로그인 시 : 로컬스토리지에서 auth 가져와서 저장
    const authId = localStorage.getItem('auth');
    setId(authId);
  }, []);

  const handleDateChange = (date) => { //date-picker에서 날짜 선택할 때마다 선택한 날짜로 상수 변경 후 저장
    setSelectedDate(date);
  };
  useEffect(() => { //날짜 선택할 때마다 데이터 보내기 위해 끝부분에 [selectedDate] 추가
    sendExerciseData();
  }, [selectedDate]);

  const sendExerciseData = async () => { //서버에 데이터 보내기(id, date 정보)
    try {
  ...
  const handleAddExercise = () => {  //모달창 띄우고 닫는 부분
    setShowModal(true);
  };
